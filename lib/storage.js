...
const { collapseTranslations, collectReferences, transform, postTransform, stringToTerms } = require('./filters');
...
class Storage {
  ...
  query(db, values = {}, options = {}) {
    const langcode = options.langcode || null;
    const filtered_sections = options.with ? this.sections.filter((s) => options.with.indexOf(s) == -1) : this.sections;

    const query = db.distinct().from(this.table)
    ...
    for (let name in values) {
      if (!this.filters.has(name)) {
        console.warn(`Query contains inexistent filter '${name}'`);
        continue;
      }
...
module.exports = { Storage, processFilters };
