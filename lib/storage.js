...
function partialMatchFilter (query, field, value) {
  query.whereRaw(`${field} ILIKE ?`, [`%${value}%`])
}
...
class ServicePointStorage extends Storage {
  constructor(schedules) {
    super({
      table: 'organisations',
      sections: ['buildingInfo', 'customData', 'departments', 'emailAddresses', 'links', 'mailAddress', 'persons', 'phoneNumbers', 'pictures', 'primaryContactInfo', 'services', 'transitInfo', 'librarySystem'],
      refs: {
        city: 'city',
      ...
      filters: {
        ...
        library_system_servicepoint_id: {
          filter: 'terms',
        },
        library_system_name: {
          filter: 'terms',
