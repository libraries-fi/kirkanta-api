...
class AbstractStorage {
  ...
  search(params) {
    return this.query(this.compile(params));
  }
...
class PostgresStorage extends AbstractStorage {
  ...
  query(query) {
    let transformables = this.transformableFields;
    console.log("T", transformables);
    return database.postgres().search(query).then(result =>result.map(row => {
      transformables.forEach(field => {
        let callback = this.config.schema[field].transform;
...
class OrganisationStorage extends ElasticStorage {
  search(params) {
    let storage = this;
    let query = this.compile(params);
    ...
    return this.query(query).then(organisations => {
      if (organisations.items.length == 0) {
        storage.mergeSchedules(organisations.items, []);
        ...
        return storage.searcher.search("opening_time", foo).then(schedules => {
          storage.mergeSchedules(organisations.items, schedules.items);
          return organisations;
        }, error => {
          console.error("OrganisationStorage:", error.stack);
  ...
  mergeSchedules(organisations, schedules) {
    let cache = new Map;
