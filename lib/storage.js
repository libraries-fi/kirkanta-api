  ...
  query(db, user_input = {}, options = {}) {
    ...
    for (let name in values) {
      if (!this.filters.has(name)) {
        console.warn(`No rule for parameter '${name}'.`)
        continue;
      }
    ...
    const sorting = options.sort.length ? options.sort : this.__options.defaultSorting;
...
class Storage extends BasicStorage {
  ...
  load(db, values, options) {
    return this.query(db, values, options).transform((row) => row.doc);
