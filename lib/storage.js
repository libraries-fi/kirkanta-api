...
class ElasticStorage extends AbstractStorage {
  compile(params) {
    ...
    try {
      Object.keys(params).forEach(key => {
        ...
        let value = rule.use_raw ? params[key] :
          rule.multiple ?
            params[key].split(/[\s,]+/).map(val => val.trim().toLowerCase()) :
        ...
        if (rule.query || rule.filter) {
          ...
          let query = {
            [rule.query || rule.filter]: callback.call(null, field, value, params)
          };


          if (query[rule.query || rule.filter]) {
            if (negate) {
    ...
    if ("sort" in params) {
      let sorting = params.sort.split(/[\s,]+/).map(key => {
...
class PostgresStorage extends AbstractStorage {
  ...
  compile(params, with_select) {
    ...
    if (with_select) {
      if ("sort" in params) {
        try {
          params.sort.split(/[\s,]+/).forEach(field => {
    ...
    Object.keys(schema).forEach(key => {
      ...
      if (key in params) {
        ...
        let value = defs.filter == "prefix" ? params[key] + "%" : params[key].split(/[\s,]+/);

        if (defs.multilang && params.lang && params.lang != "fi") {
          let clause = Array.isArray(value)
            ? util.format("translations->'%s'->>'%s' IN (?)", params.lang, field)
            : util.format("translations->'%s'->>'%s' %s ?", params.lang, field, op)
            ;
