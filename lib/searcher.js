...
let organisations = searcher.addStorage(new storage.ElasticStorage({
  ...
  sections: ["services", "accessibility", "phone_numbers", "pictures", "links", "link_groups", "persons", "extra", "mail_address", "meta"],
  references: [
    {
      name: "region",
      field: "city.region",
  ...
  filters: {
    ...
    "consortium.name": {
      ...
      multiple: true,
      multilang: true,
      field: "consortium.name",
    },
    "consortium.slug": {
      filter: "terms",
      multiple: true,
      multilang: true,
      field: "consortium.slug",
    },
    service: {
      field: "services.id",
      filter: "terms",
      ...
      script: (field, value, params) => {
        ...
        return {
          distance: {
            ...
            script: "doc['address.coordinates'].arcDistanceInKm(lat, lon)"
          }
        };
      },
    },
    q: {
      query: "query_string",
      callback: (field, value, params) => {
        let langcode = params.lang || config.api.default_language;
        let fields = ["name", "short_name", "city.name", "consortium.name", "services.name", "address.area", "slug"];

        value = value.toLowerCase().trim();

        if (value.length) {
          let qs = value.split(/\s+/).map(w => w + "*").join(" OR ") + " OR " + value;

          let filters = {
            fields: fields.map(f => f + "." + langcode),
            query: qs
          };
...
organisations.on("result", event => {
  if (event.params.with.indexOf("schedules") != -1 && event.result.items.length > 0) {
    let params = {
...
schedules.on("result", event => {
  if (event.params.refs.indexOf("period") != -1) {
    ...
    event.result.items.map(day => {
      ...
      // Mobile library days do not store period data.
      if (day.period) {
...
let consortiums = searcher.addStorage(new storage.ElasticStorage({
  type: "consortium",
  index: config.elastic.index,
  sections: ["finna", "link_groups"],
  filters: {
    id: {
      query: "ids",
      field: "_id",
...
let consortiums_pgsql = searcher.addStorage(new storage.PostgresStorage({
  type: "consortium_pgsql",
  table: "consortiums",
  join: [
    {
      type: "left_join",
      to: "finna_consortium_data",
      left: "consortiums.id",
      right: "finna_consortium_data.consortium_id"
    }
  ],
  translated: true,
  schema: {
    ...
    finna_usage_info: {
      field: "finna_consortium_data.usage_info"
    },
    finna_notification: {
      field: "finna_consortium_data.notification"
    }
  }
}));

consortiums_pgsql.on("load", consortium => {
