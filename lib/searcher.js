...
const service_points = new Storage({
  ...
  filters: {
    ...
    'geo.pos': (query, value, params, options) => {
      ...
      // NOTE: Distances calculated with ST_Distance are twice the actual distances on a sphere.
      query.select(query.client.raw(`ST_Distance(addresses.coordinates, ST_Point(${lat}, ${lon})) / 2 AS distance`));
      query.innerJoin('addresses', 'organisations.address_id', 'addresses.id');
      query.andWhereRaw(`ST_Distance(addresses.coordinates, ST_Point(${lat}, ${lon})) < ?`, distance * 2);
