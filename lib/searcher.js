...
class Searcher {
  ...
  async fetchBy(type, field, value, options = {}) {
    let result = await this.search(type, {[field]: value}, options);
    ...
    if (result.items.length > 0) {
      ...
      throw new Error(`Document with slug "${slug}" of type ${type} not found.`);
    }
  }
}
...
const service_points = new Storage({
  table: 'organisations',
  sections: ['departments', 'emailAddresses', 'links', 'mailAddress', 'persons', 'phoneNumbers', 'pictures', 'primaryContactInfo', 'services', 'transitInfo'],
  refs: {
    city: 'city',
