...
const service_points = new Storage({
  table: 'organisations',
  sections: ['departments', 'mailAddress', 'persons', 'phoneNumbers', 'pictures', 'services'],
  refs: {
    city: 'city',
...
class SchedulesStorage extends BasicStorage {
  ...
  queryBuilder(db, options = {}) {
    ...
    return super.queryBuilder(db, options)
      .select('library', db.raw('MAX(period) period'), db.raw('date(opens)::text AS date'))
      .select(db.raw('CASE WHEN COUNT(*) = 1 AND MAX(closes) IS NULL THEN true ELSE false END AS closed'))
      .select(db.raw(`
        CASE
