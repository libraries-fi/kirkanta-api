...
ResultEncoder.prototype = {
  ...
  isMultilang: function(data) {
    return data instanceof Object && data && "fi" in data;
  }
};
...
XmlEncoder.prototype.encodeMultilang = function(data) {
  return Object.keys(data).map(function(lang) {
    let value = data[lang] === null ? "" : data[lang];
...
XmlEncoder.prototype.encodeField = function(field, value) {
  ...
  if (Array.isArray(value)) {
    data[1] = value.map(this.encodeField, this).join("\n");
  } else if (value === null) {
    data[1] = "";
  } else if (value instanceof Object) {
    if (this.isMultilang(value)) {
      data[0] = util.format("<%s multilang=\"true\">", field);
...
XmlEncoder.prototype.encode = function(result) {
  ...
  return new Promise(function(resolve, reject) {
    if (result.collection) {
      ...
      let data = [
        util.format("<collection total=\"%d\">", result.data.total),
        items.join(""),
        "</collection>"
      ];
    ...
    if (encoder.isCollection(result)) {
      let items = result.items.map(function(item) {
        return encoder.encodeField(encoder.type, item);
      });
      let data = [
        "<collection>",
        encoder.encodeField("total", result.total),
        util.format("<items>%s</items>", items.join("")),
        "</collection>"
      ];

      resolve(data.join(""));
    } else {
      resolve(encoder.encodeField(encoder.type, result));
    }
  });
};
...
exports.encodeResponse = function(req, res, next) {
  if ("result" in res.locals) {
    res.locals.encoder.encode(res.locals.result).then(function(data) {
