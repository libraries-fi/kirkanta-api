  ...
  compactLanguage(data, lang) {
    Object.keys(data).forEach(key => {
      if (Array.isArray(data[key])) {
        data[key].forEach(item => this.compactLanguage(item, lang));
      } else if (typeof data[key] == "object" && data[key] != null) {
        /*
         * Since upgrading to Kirkanta v3, there might not be a key for active
         * so this is a quick hack to make sure languages are compacted.
         */
        if (lang in data[key] || "fi" in data[key] || "sv" in data[key]) {
          data[key] = data[key][lang] || null;
        } else {
...
class JsonEncoder extends ResultEncoder {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.lang) {
        ...
        if (result.references) {
          this.compactReferencesLanguage(result.references, this.lang);
        }
      }
  ...
  compactReferencesLanguage(refs, lang) {
    Object.keys(refs).forEach(type => {
      Object.keys(refs[type]).forEach(id => {
        this.compactLanguage(refs[type][id], lang);
      });
    });
  }
}
...
exports.selectEncoder = function(req, res, next) {
  ...
  let options = {
    refs: "refs" in req.query ? req.query.refs : [],
  };
  ...
  switch (type) {
    ...
    case "json":
      if (req.query.callback) {
        ...
        throw new Error("Type 'jsonp' requires a callback function name");
      }
